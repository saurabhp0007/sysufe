...
const appSetting = {
  databaseURL: "https://pype-5c119-default-rtdb.firebaseio.com"
}
...
export default function Story() {
  ...
  const [isStarIcon, setIsStarIcon] = useState(true);

  const toggleIcon = () => {
    setIsStarIcon(prevState => !prevState); // Toggle local state

    // Update 'star' property for the specific item in the database
    const itemId = item.id; // Assuming 'item' contains an 'id' field
    const itemCategory = item.category; // Assuming 'item' contains a 'category' field

    update(ref(database, `List/${itemCategory}/${itemId}`), { star: !isStarIcon })
      .then(() => {
        console.log('Star property updated successfully in the database');
      })
      .catch((error) => {
        console.error('Error updating star property:', error);
      });
  };
    ...
    useEffect(() => {
      const handleClickOutside = (event) => {
        if (containerRef.current && !containerRef.current.contains(event.target)) {
          // Click occurred outside of the container, close the dropdown
          setShow4(false);
          setShow3(false);
        }
      };
            ...
            <div className='item-section' key={item[2]} style={isRevealed ? revealMain : {}}>
              ...
              <div className='star'>
                ...
                {isStarIcon ? (
                  <CiStar
                    className='star-icon'
                    style={{ fontSize: '50px', verticalAlign: 'middle', cursor: 'pointer' }}
                    onClick={toggleIcon}
                  />
                ) : (
                  <FaStar
    ...
    function sorted(mappable) {
      ...
      const renderedItems = sortedMappable.map((items, index) => {
        const random = Math.random() * 4;
        return (
        ...
        <div className='story-section'>
          ...
          <form className='section-1' >
            ...
            {content && <div className='section-1-content' >
              ...
              <div id="snackbar">Story published</div>
            </div>}
          </form>
          ...
          <section className='section-2'>
            ...
            <div className='section-2-head'>
              ...
              <div className='looking' ref={containerRef}>
